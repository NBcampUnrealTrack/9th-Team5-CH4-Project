 ...
 */
UCLASS()
class DEEPRAIDERS_API ADRZiplineRope
	: public AActor,
	  public IDRInteractableInterface
{
	GENERATED_BODY()
...
private:
	...
	bool ShouldAutoTargetEndpointB(
		const ADRPlayerCharacter* Character,
		const FVector& EndpointA,
		const FVector& EndpointB) const;

	void RefreshRopeGeometry();

	static FVector ClosestPointOnSegmentToViewRay(
...
private:
	...
	UPROPERTY(
		...
		Category = "Zipline|Ride",
		meta = (AllowPrivateAccess = "true"))
	EDRZiplineRideMode RideMode =
		EDRZiplineRideMode::AutoTraverse;
	...
	UPROPERTY(
		EditAnywhere,
		BlueprintReadOnly,
		Category = "Zipline|Ride|Manual",
		meta = (AllowPrivateAccess = "true"))
	EDRZiplineManualControlMode ManualControlMode =
	...
	FVector CachedEndpointA =
		FVector(
			TNumericLimits<float>::Max());

	FVector CachedEndpointB =
		FVector(
			TNumericLimits<float>::Max());
