...
ADRZiplineRope::ADRZiplineRope()
{
	...
	CableVisual = CreateDefaultSubobject<USplineMeshComponent>(TEXT("CableVisual"));

	CableVisual->SetupAttachment(Root);
	CableVisual->SetMobility(EComponentMobility::Movable);
	CableVisual->SetCollisionEnabled(ECollisionEnabled::NoCollision);
	CableVisual->SetGenerateOverlapEvents(false);
	CableVisual->SetForwardAxis(ESplineMeshAxis::X, false);
}

void ADRZiplineRope::OnConstruction(const FTransform& Transform)
{
...
bool ADRZiplineRope::CanInteract_Implementation(APawn* Interactor) const
{
	return CanStartZiplineRide(Interactor);
}

bool ADRZiplineRope::Interact_Implementation(APawn* Interactor)
{
	if (!HasAuthority() || !CanStartZiplineRide(Interactor))
	{
		return false;
	}
	...
	if (!ResolveEndpointLocations(LocationA, LocationB))
	{
		return false;
	}
	...
	{
		const bool bTargetEndpointB = ShouldAutoTargetEndpointB(Character, LocationA, LocationB);
	 ...
	 */
	FVector FacingDirection = Character->GetActorForwardVector().GetSafeNormal2D();

	if (RideMode == EDRZiplineRideMode::AutoTraverse)
	{
		const FVector AutoFacing = (State.GetZiplineRideTargetLocation() - State.GetZiplineRideStartLocation()).GetSafeNormal2D();
	...
	{
		FVector ManualFacing = State.GetZiplineManualPositiveAxis();

		if (ManualControlMode == EDRZiplineManualControlMode::ViewRelative)
		{
			FVector ViewDirection = Character->GetActorForwardVector().GetSafeNormal();
			...
			if (const AController* Controller = Character->GetController())
			{
				...
				const FVector ControllerViewDirection = ViewRotation.Vector().GetSafeNormal();
			...
			float ViewDot = FVector::DotProduct(ViewDirection, ManualFacing);

			if (FMath::Abs(ViewDot) < 0.1f)
			{
				ViewDot = FVector::DotProduct(Character->GetActorForwardVector(), ManualFacing);
			}
	...
	State.ZiplineFacingDirection = FacingDirection.GetSafeNormal();
	...
	{
		State.ZiplineAcceleration = ManualAcceleration;
		State.ZiplineBrakingDeceleration = ManualBrakingDeceleration;
	...
	*Movement->Velocity.ToCompactString(),
	...
	else
	{
		...
		Movement->SetZiplineRailSpeed(State.ZiplineInitialSpeed);
...
bool ADRZiplineRope::GetInteractionLocation_Implementation(APawn* Interactor, FVector& OutInteractionLocation) const
{
	FVector LocationA;
	FVector LocationB;

	if (!ResolveEndpointLocations(LocationA, LocationB) || !IsValid(Interactor))
	{
		return false;
	}

	const AController* Controller = Interactor->GetController();
	...
	if (IsValid(Controller))
	{
		...
		{
			OutInteractionLocation = ClosestPointOnSegmentToViewRay(LocationA, LocationB, ViewLocation, ViewDirection);

			return !OutInteractionLocation.ContainsNaN();
		}
	}

	OutInteractionLocation = FMath::ClosestPointOnSegment(Interactor->GetActorLocation(), LocationA, LocationB);

	return !OutInteractionLocation.ContainsNaN();
}

bool ADRZiplineRope::CanStartZiplineRide(APawn* Interactor) const
{
	...
	if (FVector::DistSquared(LocationA, LocationB) <= FMath::Square(MinRopeLength))
	{
		return false;
	}
...
bool ADRZiplineRope::ShouldAutoTargetEndpointB(const ADRPlayerCharacter* Character, const FVector& LocationA, const FVector& LocationB) const
{
	if (!IsValid(Character))
	{
		return true;
	...
	const FVector MountPoint = FMath::ClosestPointOnSegment(Character->GetActorLocation(), LocationA, LocationB);

	const float EndpointSnapDistanceSquared = FMath::Square(AutoEndpointSnapDistance);

	/*
	 * Endpoint 바로 옆에서 잡은 경우에는
	 * 항상 반대편으로 출발시켜 기존 Endpoint UX를 보존한다.
	 */
	if (FVector::DistSquared(MountPoint, LocationA) <= EndpointSnapDistanceSquared)
	{
		return true;
	}

	if (FVector::DistSquared(MountPoint, LocationB) <= EndpointSnapDistanceSquared)
	{
		return false;
	}
	...
	const FVector ToA = (LocationA - MountPoint).GetSafeNormal();
	const FVector ToB = (LocationB - MountPoint).GetSafeNormal();

	const float DotA = FVector::DotProduct(ViewDirection, ToA);
	const float DotB = FVector::DotProduct(ViewDirection, ToB);

	if (!FMath::IsNearlyEqual(DotA, DotB, 0.01f))
	{
		return DotB > DotA;
	}
	 ...
	 */
	const FVector RopeAxis = (LocationB - LocationA).GetSafeNormal();
...
void ADRZiplineRope::RefreshRopeGeometry()
{
	...
	if (LocationA.Equals(CachedEndpointA, 0.1f) && LocationB.Equals(CachedEndpointB, 0.1f))
	{
		return;
	}
	...
	const FVector Segment = LocationB - LocationA;
	...
	InteractionVolume->SetWorldLocationAndRotation(MidPoint, SegmentDirection.Rotation());
	InteractionVolume->SetBoxExtent(
		FVector(
			SegmentLength * 0.5f + FMath::Max(InteractionEndPadding, 0.f), 
			FMath::Max(InteractionHalfWidth, 1.f), 
			FMath::Max(InteractionHalfWidth, 1.f))
			);
	InteractionVolume->SetCollisionEnabled(ECollisionEnabled::QueryOnly);

	const FTransform ActorTransform = GetActorTransform();
	const FVector LocalStart = ActorTransform.InverseTransformPosition(LocationA);
	const FVector LocalEnd = ActorTransform.InverseTransformPosition(LocationB);
	const FVector LocalTangent = LocalEnd - LocalStart;

	CableVisual->SetStaticMesh(CableMesh);
	CableVisual->SetForwardAxis(ESplineMeshAxis::X, false);
	CableVisual->SetStartAndEnd(LocalStart, LocalTangent, LocalEnd, LocalTangent, false);

	const FVector2D SafeCableScale(FMath::Max(CableScale.X, 0.01f), FMath::Max(CableScale.Y, 0.01f));
	CableVisual->SetStartScale(SafeCableScale, false);
	CableVisual->SetEndScale(SafeCableScale, false);
	CableVisual->SetMaterial(0, CableMaterialOverride);
	CableVisual->SetVisibility(IsValid(CableMesh), true);
	CableVisual->MarkRenderStateDirty();
}

FVector ADRZiplineRope::ClosestPointOnSegmentToViewRay(const FVector& SegmentStart, const FVector& SegmentEnd, const FVector& ViewLocation, const FVector& ViewDirection)
{
	const FVector Segment = SegmentEnd - SegmentStart;
	const float SegmentLengthSquared = Segment.SizeSquared();
	const FVector RayDirection = ViewDirection.GetSafeNormal();
	if (SegmentLengthSquared <= KINDA_SMALL_NUMBER || RayDirection.IsNearlyZero())
	{
		return SegmentStart;
	}
	 ...
	 */
	const FVector W = SegmentStart - ViewLocation;

	const float B = FVector::DotProduct(Segment, RayDirection);
	const float D = FVector::DotProduct(Segment, W);
	const float E = FVector::DotProduct(RayDirection, W);
	...
	{
		SegmentAlpha = (B * E - D) / Denominator;
	}
	else
	{
		// 거의 평행한 경우 ViewLocation에 가장 가까운 segment 위치부터 시작.
		SegmentAlpha = -D / SegmentLengthSquared;
	}

	SegmentAlpha = FMath::Clamp(SegmentAlpha, 0.f, 1.f);
	FVector SegmentPoint = SegmentStart + Segment * SegmentAlpha;
	const float RayDistance = FMath::Max(0.f, FVector::DotProduct(SegmentPoint - ViewLocation, RayDirection));
	const FVector RayPoint = ViewLocation + RayDirection * RayDistance;
	SegmentAlpha = FMath::Clamp(FVector::DotProduct(RayPoint - SegmentStart, Segment) / SegmentLengthSquared, 0.f, 1.f);
	return SegmentStart + Segment * SegmentAlpha;
}
